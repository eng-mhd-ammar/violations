...
export class RolePrismaRepository extends BaseRepository<Role, RoleAttributes> implements RoleRepository
{
    ...
    constructor(private readonly prisma: PrismaService) {
        super(prisma.db.orm.public.Role);
    }
    ...
    async find(id: number, options: QueryOptions = {}): Promise<Role | null> {
        return super.find(id, options);
    }

    async findOneBy(conditions: Record<string, unknown>, options: QueryOptions = {}): Promise<Role | null> {
        return super.findOneBy(conditions, options);
    }

    async first(options: QueryOptions = {}): Promise<Role | null> {
        return super.first(options);
    }

    async update(id: number, data: Partial<RoleAttributes>): Promise<Role> {
        const updated =
    ...
    protected toDomain(data: RoleAttributes): Role {
        return new Role(data);
    }

    private toPrismaUpdateData(data: Partial<RoleAttributes>): Record<string, unknown> {
        return {
